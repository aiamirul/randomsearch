...
def parallel_job(vid_name,nh,aboutsignal,xtrigger,configPath,dst="/resultsParalel"):
    ...
    ap = FranceThoroughbredBase(in_filename=video_file, dst=dst, model=objects_pd ,debug=1,triger_info=triger_info,xtrigger=triger_info,aboutsignal=aboutsignal, frame_interval=3.0, nh=nh, vis=0,
...
if __name__ == '__main__':
    ...
    trackids =["045"]
    triger_info=[]
    config_list = glob.glob(r'C:\Users\Amir\Documents\GitHub\mzmses\randomsearch\randomsearch\Experiment\Configs\**\*.json')
    print("Config Samples ",len(config_list))

    for configSample in config_list:
        triger_info=[]
        print(configSample)
        #saveResultPat
        Configpath = Path(configSample)
        print(Configpath.parent.absolute())
        saveResPath = str(Configpath.parent.absolute())
        #results = Parallel(n_jobs=multiprocessing.cpu_count())(delayed(parallel_job)(video_list[i],nh_list[i],aboutsignals_list[i],triggers_list[i],configSample,dst_path) for i in range(len(video_list)))
