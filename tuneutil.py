...
def GetAllConfigScore(experimentFolder,n_gen=0):
    print(experimentFolder+r"/Configs--G"+str(n_gen)+r"/**/*.csv")
    resStack = glob.glob(experimentFolder+r"/Configs--G"+str(n_gen)+r"/**/*.csv")
    ...
    df
    return df
def findMate(cfg,matepool):
    print("To be married: ",cfg)
    matepool = matepool[matepool != cfg]
    #print("Matepool",list(matepool))
    return random.choice(list(matepool))
def mutation(data,active_genes,rate=0.04,rate_multiplier=0.25,max_multiplier=4):
    rate = rate*100
    flag = False
    for gene in active_genes:
        if(random.randint(0,100) >= (100-rate)):
            print(gene)
            a = data['param'][0][gene]
            #weight of mutation & chance of extra weight
            if(random.randint(0,100) >= (100-rate_multiplier)):
                multiplier = int(random.randint(0,100*max_multiplier)/100)
            else:
                multiplier =1

            #Direction of mutation 50:50
            if(random.randint(0,100) > 50):
                if(a > 1):
                    b = a + int(1*multiplier)
                else:
                    b = a + round(0.1*multiplier,1)
            else:
                if(a > 1):
                    b = a - int(1*multiplier)
                else:
                    b = a - round(0.1*multiplier,1)
            print("Mutated {} -> {}".format(str(a),str(b)))
            data['param'][0][gene] = b
            flag = True
    return data,flag
...
def Generate_new_gen(envFolder,active_genes,current_gen=0,nextGenRate=0.25):
    n_gen=""
    if(current_gen >= 0):
        n_gen = str(current_gen)
    newgen = str(current_gen+1)
    print("load all scores")
    df = GetAllConfigScore(envFolder,current_gen)
    
    #N-Generation = 25% of n-1 generation
    top_n = int(len(df)*nextGenRate)
    print(df)
    print("load all scores")
    bestlist = df.sort_values(['Score'], ascending=[False]).head(top_n)
    matepool = bestlist['Config']
    for cfg in bestlist['Config']:
        print(cfg)
        jsonfile = '{}/{}/{}/{}'.format(envFolder,"Configs--G"+str(current_gen),cfg,"config.json")
        with open(jsonfile, 'r') as f:
            data = json.load(f)
        #print(data)
        mate = findMate(cfg,matepool)
        print(mate)

        jsonfile = '{}/{}/{}/{}'.format(envFolder,"Configs--G"+str(current_gen),mate,"config.json")
        filename = jsonfile
        with open(filename, 'r') as f:
            matedata = json.load(f)
        #print(matedata)
        childdata=data
        for genes in active_genes:
            a = data['param'][0][genes]
            b = matedata['param'][0][genes]
            if(random.randint(0,1)):
                c = a
            else:
                c = b
            childdata['param'][0][genes] = c

        mutateddata,flag = mutation(childdata,active_genes)
        if(flag):
            new = mutateddata
        else:
            new = childdata

        #make new generation configs
        newID = "{}00{}0{}".format(current_gen+1,cfg,mate)
        newgen_dir  = '{}/{}/{}'.format(envFolder,"Configs--G"+str(current_gen+1),newID)
        os.makedirs(newgen_dir, exist_ok=True)
        savejson = "{}/config.json".format(newgen_dir)
        with open(savejson, "w") as outfile:
            outfile.write(json.dumps(new))


def parallel_job(detection_dir,video_dir,vid_name,nh,aboutsignal,xtrigger,configPath,dst="/resultsParalel"):
    ...
    if(not os.path.isfile(video_file)):
        ...
        raceid=os.path.splitext(os.path.basename(video_file))[0]
        return [str(trackid),raceid,"-99","-99","-99","-99","-99","-99"]   
    if( not cv2.VideoCapture(video_file).isOpened()):
        print(video_file, "corrupted video")
        raceid=os.path.splitext(os.path.basename(video_file))[0]
        return [str(trackid),raceid,"-98","-98","-98","-98","-98","-98"]
    triger_info=[]
    objects_pd = pd.read_csv(detection_file)
    ap = FranceThoroughbredBase(in_filename=video_file, dst=dst, model=objects_pd ,debug=1,triger_info=triger_info,xtrigger=xtrigger,aboutsignal=aboutsignal, frame_interval=3.0, nh=nh, vis=0,
                        in_buffer=sys.stdin, width=640, height=480, raceid=str(trackid), program='QVD', test_mode=1,config_path=configPath)
    ...
    return ap.triger_info[0]


def runSim(configSample):
    print("RunThisSample",configSample)
    from pathlib import Path
    configSamplepath = Path(configSample)
    Exppath = configSamplepath.parent.absolute().parent.absolute()
    print(Exppath)
    WIN=1
    ...
    csv_dir = str(Exppath)+r"/045resultx.csv"
    video_list,nh_list,aboutsignals_list,triggers_list=get_list_and_nh(infile_csv=csv_dir)
    trackids =["045"]
    triger_info=[]
    print(configSample)
    #saveResultPat
    Configpath = Path(configSample)
    print(Configpath.parent.absolute())
    saveResPath = str(Configpath)
    print("saving to ",saveResPath)
    results = Parallel(n_jobs=multiprocessing.cpu_count())(delayed(parallel_job)(detection_dir,video_dir,video_list[i],nh_list[i],aboutsignals_list[i],triggers_list[i],configSample,dst_path) for i in range(len(video_list)))
    
    print(results)
    utl.write_to_csv(info=results,name=saveResPath+str(r"/result"),header=["track","raceid","trigger","Xtrigger",'nh','gate','about','property'])  
...
class FranceThoroughbredBase:
    def __init__(self,in_filename,dst,frame_interval,model=None,debug=0,xtrigger=-1,aboutsignal=-1,triger_info=None,nh=15,vis=0, in_buffer=None, width=None, height=None, raceid=None, program=None, test_mode=1,config_path="config.json"):
